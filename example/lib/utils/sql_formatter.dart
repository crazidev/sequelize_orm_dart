...
class SqlFormatter {
  ...
  static const String _keyword = '\x1B[94m'; // Bright blue
  static const String _operator = '\x1B[93m'; // Bright yellow
  static const String _identifier = '\x1B[96m'; // Bright cyan
  ...
  /// Adds color highlighting to SQL
  static String addColors(String sql) {
    String colored = sql;
    ...
    for (final keyword in keywords) {
      colored = colored.replaceAllMapped(
        ...
        (match) => '$_keyword${match.group(0)}$_reset',
      );
    }
    ...
    colored = colored.replaceAllMapped(
      RegExp(r'"([^"]+)"'),
      (match) => '$_identifier"${match.group(1)}"$_reset',
    );

    // Color operators
    colored = colored.replaceAllMapped(
      RegExp(r'([=<>!]+)'),
      (match) => '$_operator${match.group(1)}$_reset',
    );
  ...
  static void printFormatted(String sql, {String? prefix}) {
    final formatted = format(sql);
    final colored = addColors(formatted);
    if (prefix != null) {
      print('$prefix\n$colored');
    } else {
      print(colored);
